    ...
    <>
      ...
      <div className="absolute left-8 top-4 z-20">
        <Image
          ...
          // We intentionally do NOT invert the brand logo as requested, assuming it stays intact or has its own design.
        />
      </div>
      ...
      <div className="absolute right-8 top-6 z-20">
        {mounted && (
          <div
            className="flex items-center rounded-full bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 p-1 backdrop-blur-md"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Sun icon / Light theme toggle */}
            <button
              type="button"
              className={`flex h-8 w-8 items-center justify-center rounded-full transition-colors ${theme === 'light' ? 'bg-black/10 text-black shadow-sm' : 'text-white/40 hover:text-white'}`}
              title="Light mode"
            ...
            <button
              type="button"
              className={`flex h-8 w-8 items-center justify-center rounded-full transition-colors ${theme === 'dark' ? 'bg-white/20 text-white shadow-sm' : 'text-black/40 hover:text-black'}`}
              title="Dark mode"
