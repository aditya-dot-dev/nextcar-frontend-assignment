...
export default function ThankYouScreen({ flowState, setFlowState, shrikhandClassName }: ThankYouScreenProps) {
  return (
    <>
      ...
      >
        <h2 
          className={`absolute top-0 left-0 -translate-x-1/2 -translate-y-1/2 pointer-events-none ${shrikhandClassName} text-[#F4F4F4] font-normal text-[60px] leading-[39px] tracking-[0.01em] text-center`}
          style={{ width: '387px' }}
        >
          THANK YOU
        </h2>
        <button
          className={`absolute left-0 w-[135px] h-[49px] flex items-center justify-center rounded-[30px] bg-white/20 hover:bg-white/30 transition-all duration-300 text-white text-base font-normal`}
